...
class Trainer(CIFARTrainer):
    def set_criterion(self):
        ...
        for i in range(10):
            sub_index=np.load(f"/data/home/huqiang/DeepCore/save/Neighbor/sorted_indices{i}_10.npy")
            self.neighbor_indices.extend(sub_index)

        num_examples = len(self.neighbor_indices)
        min_weight = 0.7
        max_weight = 1.3
        ...
        for i, neighbors in enumerate(self.neighbor_indices):
            weight = (max_weight - min_weight) * ((len(self.neighbor_indices)-1-i) / num_examples) + min_weight
            self.weights[neighbors] = weight
