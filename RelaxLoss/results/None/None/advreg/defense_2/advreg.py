...
class Attack(nn.Module):
    def __init__(self, input_dim, num_classes=1, hiddens=[100]):
        ...
        self.last_layer = nn.Linear(hiddens[-1], num_classes).to(self.device)
...
class Trainer(CIFARTrainer):
    def __init__(self, the_args, save_dir):
        ...
        self.set_specific()
        # self.attack_member_loader = the_args.attack_member_loader
        # self.attack_nonmember_loader=the_args.attack_nonmember_loader
    ...
    def set_specific(self):
        ...
        self.attack_nonmember_loader = torch.utils.data.DataLoader(attack_nonmember_data,
                                                                   ...
                                                                   shuffle=True, num_workers=self.args.num_workers, generator=torch.Generator(device='cuda'))
    ...
    def train_privately(self, target_model, attack_model, target_optimizer, num_batches=10000, num_loaders_to_defend=2):
        ...
        with open('/data/home/huqiang/DeepCore/save/subset_loaders.pkl', 'rb') as f:
            loaded_dataloader_list = pickle.load(f)
        idx=np.arange(len(loaded_dataloader_list)).astype(int)
        np.random.shuffle(idx)
        for loader_idx in idx:
            for batch_idx, (inputs, targets) in enumerate(loaded_dataloader_list[loader_idx]):
                ...
                if loader_idx > num_loaders_to_defend:
                    # Apply defense only to the specified number of batches
                    ...
                    # For the remaining batches, proceed without applying defense
                    outputs = target_model(inputs)
